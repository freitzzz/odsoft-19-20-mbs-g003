...
import pt.isep.cms.teachers.client.event.ViewTeacherDetailsEventHandler;
...
public class TeachersController implements Presenter { // (ATB) No history at this level, ValueChangeHandler<String> {
	...
	private void bind() {
		...
		eventBus.addHandler(ViewTeacherDetailsEvent.TYPE, new ViewTeacherDetailsEventHandler(){
		
			@Override
			public void onViewTeacherDetails(ViewTeacherDetailsEvent event) {
				doViewTeacherDetails(event.getId());
			}
		});
	}

	private void doViewTeacherDetails(long id) {
		// Lets use the presenter to display a dialog
		Presenter presenter = new ViewTeacherDetailsPresenter(rpcService, eventBus, new TeacherDetailsDialog(), id);
